    ...
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription>Ask the AI a question about this lecture.</CardDescription>
        </CardHeader>
              ...
              <div className="space-y-4">
                {messages.length === 0 && (
                    <div className="flex flex-col items-center justify-center text-center p-4 h-full">
                        ...
                        <p className="mt-2 text-sm text-muted-foreground">Ask a question to get started.</p>
                    </div>
                )}
    ...
    if (!liveClass || liveClass.status !== 'live' || !liveClass.script) {
        return (
            <div className="flex flex-col items-center justify-center h-full text-center">
                 ...
                 <p className="text-muted-foreground mt-2">This session is no longer active.</p>
                 <Button asChild className="mt-6">
                     <Link href="/dashboard/live-classes">Return to Live Classes</Link>
                 </Button>
            </div>
        )
    }
                ...
                <div className="lg:col-span-2">
                     <Card>
                        <CardHeader>
                            ...
                            <CardDescription>Follow along as the AI teacher presents. The current sentence is highlighted.</CardDescription>
                        </CardHeader>
                        <CardContent>
                                ...
                                <div className='prose prose-sm md:prose-base dark:prose-invert max-w-none p-2 text-base leading-relaxed'>
                                    <p>
                                        {flatSentences.map((sentence, index) => (
                                            <span key={index} id={`sentence-${index}`} className={cn(
                                                "transition-colors duration-300",
                                                index === currentSentenceIndex ? "bg-primary/20 rounded" : ""
                                            )}>
                                                {sentence}
                                            </span>
                                        ))}
                ...
                <div className="lg:col-span-1 space-y-6 lg:sticky lg:top-24">
                     <Card className="overflow-hidden">
                        <CardHeader className="text-center p-6 bg-muted/30">
                            {aiTeacherAvatar && (
                                <Avatar className="h-24 w-24 mx-auto animate-bob shadow-lg border-4 border-primary/20">
                            ...
                            <CardTitle className="font-headline mt-4">{liveClass.teacherName}</CardTitle>
                             <CardDescription className="text-sm font-medium text-green-500 animate-pulse flex items-center justify-center gap-2">
                        ...
                        <CardContent className="p-4 flex justify-center gap-2">
                            ...
                            <Button onClick={stopSpeech} variant="outline" size="icon" disabled={!isSpeaking && !isPaused} aria-label="Stop">
                                <StopCircle />
                            </Button>
                        </CardContent>
                    </Card>
